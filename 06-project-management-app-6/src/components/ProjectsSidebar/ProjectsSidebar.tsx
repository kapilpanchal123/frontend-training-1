import Button from '../Button/Button';
import type { ProjectData } from '../Data';

type Props = {
  onStartAddProject: () => void;
  projects: ProjectData[];
  onSelectProject: (id: string) => void;
  selectedProjectId: string | undefined | null;
};

const ProjectsSidebar = ({ onStartAddProject, projects, onSelectProject, selectedProjectId }: Props) => {
  return (
    <>
      <aside className="w-1/3 px-8 py-16 bg-stone-900 text-stone-50 md:w-72 rounded-r-xl">
        <h2 className="mb-8 font-bold uppercase md:text-xl text-stone-200">Your Projects</h2>
        <div>
          <Button onClick={ onStartAddProject }>
            + Add Project
          </Button>
        </div>
        <ul className="mt-8">
          {projects.map((project) => { 
            
            let cssClasses = "w-full text-left px-2 py-1 rounded-sm my-1 text-stone-400 hover:text-stone-200 hover:bg-stone-800 ";
            
            if(project.id === selectedProjectId) {
              cssClasses += "bg-stone-800 text-stone-200";
            } else {
              cssClasses += "text-stone-400";
            }

            return (
              <li key={project.id} className="py-2 border-b border-stone-700">
                <button onClick={() => onSelectProject(project.id?.toString() ?? "")} 
                  className={cssClasses}>{project.title}</button>
                <p className="text-stone-400 text-sm">{project.description}</p>
                <p className="text-stone-500 text-xs">{project.dueDate}</p>
              </li>
            )
          })}
        </ul>
      </aside>
    </>
  )
};

export default ProjectsSidebar;